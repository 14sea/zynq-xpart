...
// ---- M7.2 INERT-PADDING (2026-06-23): bloat .text+.rodata to the diag3 size class
//      WITHOUT changing ANY forward logic, to test whether pure binary size/layout
//      flips diag2's CORRECT forward {19,2,10,-1} into the {-5,-3,-8,-3} failure.
//      pad_consume() runs AFTER r0-3 are computed, so it cannot affect them. ----
__attribute__((used)) static const uint8_t PAD_RO[7000] = { 1,2,3,4,5,6,7,8,9 };
...
static uint32_t pad_consume(void){ uint32_t s=PAD_RO[0]; for(int i=1;i<7000;i++) s+=PAD_RO[i]; return pad_fn0(s)^pad_fn1(s)^pad_fn2(s); }
...
int main(void) {
    ...
    // SAME-BITSTREAM SETTLE CONTROL (M7.2): forward computed TWICE off one config —
    // immediately at boot (pre-settle, B0..B3) and again after a long settle (post-settle,
    // B4..B7). pre wrong + post correct => placement-dependent post-config SETTLE (time),
    // not the route (route is identical across pre/post; only wall-clock time differs).

    // PRE-settle forward (immediate at boot, no settle) -> r[0..3] = B0..B3
    m7_init(W1,b1,W2,b2);
    for (int s=0;s<4;s++){ i64 z1[4],h[4],z2[4],y[4];
        m7_forward(W1,b1,W2,b2,M7_XP[s],z1,h,z2,y); r[s]=(uint32_t)(int32_t)y[0]; }

    // long wall-clock settle + array warm-up (the ONLY difference between pre and post)
    { const signed char Ww[4][4]={{1,1,1,1},{1,2,3,4},{2,2,2,2},{1,0,1,0}}; const signed char Xw[4]={2,3,4,5};
      int32_t a[4]; for(int w=0;w<256;w++) array_macc(Ww,Xw,a);
      for(volatile uint32_t d=0;d<150000000u;d++){} }

    // POST-settle forward (same config) -> r[4..7] = B4..B7
    m7_init(W1,b1,W2,b2);
    for (int s=0;s<4;s++){ i64 z1[4],h[4],z2[4],y[4];
        m7_forward(W1,b1,W2,b2,M7_XP[s],z1,h,z2,y); r[4+s]=(uint32_t)(int32_t)y[0]; }

    (void)pad_consume();   // keep PAD bloat linked so size stays in the failing class
    neorv32_uart0_printf("ctrl done\n");
    while(1){ for(int i=0;i<8;i++){ MBOX=((uint32_t)(0xB0+i)<<24)|((uint32_t)r[i]&0x00FFFFFF);
