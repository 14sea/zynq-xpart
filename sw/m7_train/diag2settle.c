...
__attribute__((used)) static const uint8_t PAD_RO[4400] = { 1,2,3,4,5,6,7,8,9 };
...
static uint32_t pad_consume(void){ uint32_t s=PAD_RO[0]; for(int i=1;i<4400;i++) s+=PAD_RO[i]; return pad_fn0(s)^pad_fn1(s)^pad_fn2(s); }
...
int main(void) {
    ...
    // SETTLE TEST (M7.2 lead): long wall-clock settle + array warm-up BEFORE the forward.
    // If the bad build's forward now reads {19,2,10,-1}, the divergence is a placement-
    // dependent post-config settle (fixable by waiting), not the in-context route.
    { const signed char Ww[4][4]={{1,1,1,1},{1,2,3,4},{2,2,2,2},{1,0,1,0}}; const signed char Xw[4]={2,3,4,5};
      ...
      for(volatile uint32_t d=0;d<150000000u;d++){} }   // ~5x the existing settle
    ...
    r[6]=pad_consume();   // force PAD_RO + pad_fn* to link & execute; does NOT touch r[0..5]
    r[7]=r[6];            // (never published; mailbox loop only cycles i<6)
