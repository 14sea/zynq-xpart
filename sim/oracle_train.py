...
def dump_header(path, seed, k, lr_shift, epochs, fwd, wshift, xshift):
    ...
    with open(path, "w") as f:
        ...
        f.write(f"static const unsigned char M7_ORDER[{epochs}][4] = {{\n")
        f.write(",\n".join("  {" + ",".join(str(int(v)) for v in row) + "}" for row in order))
        f.write("\n};\n\n")
        f.write("// --- golden results (this oracle) for the C self-check ---\n")
