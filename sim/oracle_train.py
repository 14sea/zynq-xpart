...
def dump_header(path, seed, k, lr_shift, epochs, fwd, wshift, xshift):
    ...
    per-epoch sample order, then emit a C header so sw_src/m7_train/train_xor.c
    can replay the EXACT same fixed-point run (no RNG on the board — init + order
    are host-provided, per m7_plan.md open-decision #5/#6) and bit-exact compare
    its final weights + loss curve against this oracle (the golden ref)."""
    order_log, init_log = [], {}
    ...
    with open(path, "w") as f:
        ...
        f.write("// weights/loss for the bit-exact C-vs-oracle check (M7.0b).\n")
...
def main():
    ...
    if args.dump_header:
        dump_header(args.dump_header, args.seed, args.k, args.lr_shift, args.epochs,
                    args.fwd, args.wshift, args.xshift)
        return
