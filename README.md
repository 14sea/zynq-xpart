...
| **M7.1** | **Backward `Wᵀ·δ` on the same INT8 array** (transpose-load) — full train loop bit-exact to the oracle, `XOR 4/4`. Surfaced a real-silicon **post-config settle** bug (training right after `fpga loadb` diverges; cure is wall-clock time, not warm-up count → ~45 s settle), documented in the plan | ✅ [docs/m7_plan.md](docs/m7_plan.md) |
| **M7.2** | **The tiny-tpu trio (loss / leaky′ / SGD) in hardware** (`train_unit.v`) + master weights — a single training step **bit-exact to the oracle** on the array. Multi-epoch on the dedicated `rm_train` is the **documented 7-series-DFX limit** (a build-dependent, firmware-size-sensitive in-context-routing reproducibility issue — timing-clean yet functionally varies; placement/routing-isolation fixes exhausted) | ⚠️ [docs/m7_2_dcpdiff.md](docs/m7_2_dcpdiff.md) |
...
| **M7.4** | **Bigger workload — a real classifier trained on the fabric.** A 16(=4×4)→4→2 MNIST 0/1 net trained multi-epoch on-board (array forward + Wᵀ·δ + SW SGD), the full per-epoch curve read over the mailbox **bit-exact to the numpy oracle** (SSE 16066→1999, test acc 50%→97.5%). Sized into the verified-good build band on the first roll (the 64-8-4 MNIST stays host-only — too big for the M7.2 band) | ✅ [docs/m7_plan.md](docs/m7_plan.md) |
